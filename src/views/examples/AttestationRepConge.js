...
import RepriseCongeDoc from "documents/RepriseCongeDoc";

const AttestationRepConge = () => {

    const [attestationRepConge, setAttestationRepConge] = useState([]);
    ...
    const saveArchive = async (attestation_reprise_service) => {
        if (!archive) {
        ...
        const req = `INSERT INTO archive_att_reprise_conge (fichier_archive_att_reprise_conge,created_at_archive_att_reprise_conge,id_conge) VALUES ("${archive}","${date}",${attestation_reprise_service.id_conge}) `;
        const req_conge = `UPDATE conge SET statut_att_rep_conge ="archivé" WHERE id_conge = ${attestation_reprise_service.id_conge}`;
    ...
    /** for the pagination */
    const pageCount = Math.ceil(attestationRepConge.length/perPage);
    ...
    const filterAttestationRepConge = filter !== "" || search !== ""
    ? attestationRepConge.filter(attestation_conge => attestation_conge.statut_att_rep_conge === filter && (
    ...
    const fetchDatas = async () => {
        try {
            window.electronAPI.getAttestationRepConge();
            await window.electronAPI.retrieveAttestationRepConge((event, res) => {
                for (let index = 0; index < res.length; index++) {
                    res[index].attestation_reprise_service = JSON.parse(res[index].attestation_reprise_service)                                                
                }
                setAttestationRepConge(res);
                setTimeout(() => 
                                ...
                                <tbody>                                        
                                    ...
                                    {filterAttestationRepConge && filterAttestationRepConge.length > 0 ? !loadingSpinner && (filterAttestationRepConge.slice(offset, offset + perPage).map((att_con, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <PDFDownloadLink document={<RepriseCongeDoc 
                                                    name={att_con.attestation_reprise_service.name} 
                                                    matricule={att_con.attestation_reprise_service.matricule}
                                                    sexe={att_con.attestation_reprise_service.sexe}
                                                    poste={att_con.attestation_reprise_service.poste}
                                                    type={att_con.attestation_reprise_service.type}
                                                    decision={att_con.attestation_reprise_service.decision}
                                                    duration={att_con.attestation_reprise_service.duration}
                                                    structure={att_con.attestation_reprise_service.structure}
                                                    startDate={att_con.attestation_reprise_service.startDate}
                                            ...
                                            <td>
                                                {att_con.statut_att_rep_conge === "non archivé" ?  
                                                    <Badge color="danger">
                                                        {att_con.statut_att_rep_conge}
                                                    </Badge> 
                                                    : att_con.statut_att_rep_conge === "archivé" ? 
                                                    <Badge color="success">
                                                        {att_con.statut_att_rep_conge}
                                                    </Badge> 
                                                    : 
